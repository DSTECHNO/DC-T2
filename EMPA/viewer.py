...
# -------------------------------------------------
# STREAMLIT SETTINGS (MUST BE FIRST Streamlit call)
# -------------------------------------------------
st.set_page_config(
...
<style>
    ...
    [data-testid="stSidebar"] [data-testid="stSlider"] div[data-baseweb="slider"] div[role="slider"] {
        background: #ffffff; border: 2px solid #2563eb; box-shadow: 0 0 0 1px rgba(37, 99, 235, 0.35);
    }
...
# -------------------------------------------------
# HEAVY LOADERS (RESOURCE CACHE)
# -------------------------------------------------
@st.cache_resource
def load_case_cached(npz_path: str, vtk_path: str):
    """
    Heavy: reads NPZ (mmap) + reads VTK with PyVista.
    Cached as resource so it won't reload across tab switches/reruns.
    """
...
def load_outer_geometry_cached(vtk_path: str):
    """
    Heavy: read VTK, extract surface, triangulate.
    Cached as resource to avoid repeated pv.read() calls.
    """
    import pyvista as pv
    ...
    ig, jg, kg = triangles[:, 0], triangles[:, 1], triangles[:, 2]
...
def get_coords_and_field(mesh, T_field, U_field, field_choice: str):
    ...
    # Decide if field is cell-based or point-based
...
HF_USER = "mkuzaay"  # change if needed
...
# -------------------------------------------------
# ABOUT PAGE (NO HEAVY LOADS HERE)
# -------------------------------------------------
def render_about_page():
    st.subheader("About")

    # Use URL image (deploy-safe)
    st.image(
...
# If About selected -> render and STOP BEFORE any heavy loading
if view_tab == "About":
    render_about_page()
    st.sidebar.markdown("---")
    st.sidebar.markdown(
...
# -------------------------------------------------
# THERMAL TWIN (HEAVY LOADS START HERE)
# -------------------------------------------------
...
# Load once (cached as resource)
mesh, T_field, U_field = load_case_cached(npz_path, vtk_path)

# Extract coords + selected field
...
st.sidebar.markdown(
    ...
    <div class="sidebar-card">
        ...
        <div class="sidebar-card-metric">Displayed (3D): {displayed_points:,}</div>
    </div>
    """,
    unsafe_allow_html=True
)
...
with col1:
    ...
    # Apply range filter helper
    def apply_range_mask(xa, ya, za, fa):
        m = (fa >= value_min) & (fa <= value_max)
        return xa[m], ya[m], za[m], fa[m]

    # 3D SCATTER
    if mode == "3D Scatter":
        x_plot, y_plot, z_plot, f_plot = density_aware_downsample(x, y, z, field, max_points)
        x_plot, y_plot, z_plot, f_plot = apply_range_mask(x_plot, y_plot, z_plot, f_plot)

        fig = go.Figure()

        # Outer geometry (cached)
        try:
    ...
    # 2D SLICE (colored plane in 3D geometry)
    else:
        ...
        if axis1_s.size == 0:
            ...
            # Throttle: only re-interpolate when slice parameters change
            slice_key = (slice_axis, float(slice_coord), int(thickness_percent), int(grid_resolution),
                         float(value_min), float(value_max), field_choice)

            if "last_slice_key" not in st.session_state:
                st.session_state.last_slice_key = None
            if "last_slice_result" not in st.session_state:
            ...
            if slice_key != st.session_state.last_slice_key:
                try:
                    ...
                    st.session_state.last_slice_result = (grid_axis1, grid_axis2, grid_field_filtered)

                except Exception as e:
                    st.error(f"Slice interpolation failed: {e}")
                    st.stop()

            grid_axis1, grid_axis2, grid_field_filtered = st.session_state.last_slice_result
            A1, A2 = np.meshgrid(grid_axis1, grid_axis2)
...
with col2:
    ...
    st.markdown("""
    <style>
    .kpi-table { width: 100%; border-collapse: collapse; font-size: 14px; margin-top: 10px; margin-bottom: 10px; }
    .kpi-table th { background-color: #2c3e50; color: white; padding: 10px 6px; text-align: left; font-weight: bold; font-size: 13px; }
    .kpi-table td { padding: 10px 6px; border-bottom: 1px solid #ddd; font-size: 13px; }
    ...
    <div class="kpi-title">KPI Assessment (placeholder)</div>

    <table class="kpi-table">
        <tr><th>KPI</th><th>Value</th><th>Assessment</th></tr>
        <tr><td><strong>RCI<sub>HI</sub></strong></td><td>190.83</td><td>🔥 Hot-air recirculation</td></tr>
        <tr><td><strong>RCI<sub>LO</sub></strong></td><td>248.62</td><td>❄️ Cold-air bypass</td></tr>
        <tr><td><strong>RTI</strong></td><td>96.58</td><td>⚠️ Overcooling</td></tr>
        <tr><td><strong>RHI</strong></td><td>0.95</td><td>🔥 Moderate hot-air recirculation</td></tr>
        <tr><td><strong>RI</strong></td><td>97.82</td><td>🔥 Hot-air recirculation</td></tr>
        <tr><td><strong>CCI</strong></td><td>1.35</td><td>⚠️ Overcooling and inefficient airflow management</td></tr>
        <tr><td><strong>LI</strong></td><td>142.10</td><td>⚠️ Leakage index</td></tr>
    </table>
